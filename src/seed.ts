...
const main = async () => {
  ...
  const course = await prisma.course.create({
    data: {
      ...
      users: {
        create: {
          role: 'TEACHER',
          user: { connect: { email: grace.email } }
        }
      }
    },
    include: { tests: true }
  ...
  const davidAggregates = await prisma.testResult.aggregate({
    where: { student: { email: david.email } },
    avg: { result: true },
    max: { result: true },
    min: { result: true },
    count: true
  ...
  const shakuntalaAggregates = await prisma.testResult.aggregate({
    where: { student: { email: shakuntala.email } },
    avg: { result: true },
    max: { result: true },
    min: { result: true },
    count: true
  });
  console.log(`Shakuntala's results (email: ${david.email})`, shakuntalaAggregates);
}
